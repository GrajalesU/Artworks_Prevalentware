...
export default function FavoriteBtn({ artwork }: FavoriteBtnProps) {
  ...
  const handleAddToFav = async () => {
    try {
      setLoading(true);
      const postArtworkRes = await fetch("/api/artwork", {
        method: "POST",
      ...
      const postArtwork = (await postArtworkRes.json()) as Artwork;
      ...
      const favoriteRes = await fetch("/api/favorites", {
        method: "POST",
        body: JSON.stringify({ artwork_id: postArtwork.id }),
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  if (loading) {
    return (
      <div role="status">
        <svg
          aria-hidden="true"
          className="inline w-6 h-6 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-primary-500 dark:fill-primary-500"
          viewBox="0 0 100 101"
          fill="none"
